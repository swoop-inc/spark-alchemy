...
private[sql] trait SQLTestData { self =>
  ...
  protected lazy val emptyTestData: DataFrame = {
    val df = spark.sparkContext.parallelize(
      Seq.empty[Int].map(i => TestData1(i, i.toString))).toDF()
    df.createOrReplaceTempView("emptyTestData")
    df
  }

  protected lazy val testData1: DataFrame = {
    val df = spark.sparkContext.parallelize(
      (1 to 100).map(i => TestData1(i, i.toString))).toDF()
  ...
  protected lazy val complexData: DataFrame = {
    val df = spark.sparkContext.parallelize(
      ComplexData(Map("1" -> 1), TestData1(1, "1"), Seq(1, 1, 1), true) ::
        ComplexData(Map("2" -> 2), TestData1(2, "2"), Seq(2, 2, 2), false) ::
        Nil).toDF()
...
private[sql] object SQLTestData {
  case class TestData1(key: Int, value: String)
  ...
  case class ComplexData(m: Map[String, Int], s: TestData1, a: Seq[Int], b: Boolean)
